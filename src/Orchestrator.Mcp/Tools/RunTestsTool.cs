...
public sealed class RunTestsTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var request = new RunTestsRequest { ProjectPath = projectPath, TestFilter = string.IsNullOrWhiteSpace(filter) ? null : filter, TimeoutSeconds = timeoutSeconds };
            request.ValidateOrThrow(new RunTestsRequestValidator());
            if (string.IsNullOrWhiteSpace(allowedRoot)) return JsonSerializer.Serialize(new RunTestsResponse { Summary = new TestSummary(), Failures = [], Error = new McpError { Code = "ALLOWED_ROOT_REQUIRED", Message = "allowedRoot is required.", Retryable = false }, Meta = new Meta { TaskId = Guid.NewGuid().ToString("N"), Node = "local" } }, JsonConfig.Default);
            var fullPath = Path.GetFullPath(projectPath);
            var fullRoot = Path.GetFullPath(allowedRoot).TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar;
            if (!fullPath.StartsWith(fullRoot, StringComparison.OrdinalIgnoreCase)) return JsonSerializer.Serialize(new RunTestsResponse { Summary = new TestSummary(), Failures = [], Error = new McpError { Code = "PATH_VIOLATION", Message = $"projectPath '{projectPath}' is outside the allowed root '{allowedRoot}'", Retryable = false }, Meta = new Meta { TaskId = Guid.NewGuid().ToString("N"), Node = "local" } }, JsonConfig.Default);
            ...
            McpError? error = null;
            if (exitCode != 0 && failures.Count == 0) error = new McpError { Code = "TEST_RUN_FAILED", Message = stderr.Length > 0 ? stderr[..Math.Min(500, stderr.Length)] : "dotnet test exited with non-zero code", Retryable = false };
            return JsonSerializer.Serialize(new RunTestsResponse { Summary = summary, Failures = failures, Error = error, Meta = new Meta { TaskId = Guid.NewGuid().ToString("N"), Node = "local", LatencyMs = (int)sw.ElapsedMilliseconds } }, JsonConfig.Default);
        }
