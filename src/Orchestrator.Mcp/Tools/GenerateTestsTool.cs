...
public sealed class GenerateTestsTool
{
    ...
    public GenerateTestsTool(IRoutingService routing, IIdempotencyCache idempotency) { _routing = routing; _idempotency = idempotency; }
    ...
    private async Task<string> ExecuteCoreAsync(string code, string language, string framework, string coverage, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var prompt = BuildPrompt(code, safeLanguage, safeFramework, safeCoverage);
            var taskId = Guid.NewGuid().ToString("N");
            var result = await _routing.RouteAsync(TaskType.TestGeneration, new InferenceRequest { Prompt = prompt, Stream = true, Priority = QueuePriority.Normal }, cancellationToken);
            var response = new GenerateTestsResponse { Files = [new GeneratedTestFile { Path = $"Tests.{safeLanguage}", Content = result.Text }], Meta = Meta.FromInferenceResult(taskId, result) };
