...
{
    // Guard rail — skip any single file larger than this when collecting snippets,
    // so a stray binary blob doesn't blow up the prompt or memory.
    ...
    private static List<(string Path, string Snippet)> CollectFiles(string rootPath, string pattern, int limit)
    {
        ...
        foreach (var file in files)
        {
            try
            {
                // Size guard — skip anything larger than MaxFileSizeBytes to avoid
                // pulling binaries or massive generated files into the prompt.
