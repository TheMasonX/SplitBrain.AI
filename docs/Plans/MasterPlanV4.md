# SplitBrain.AI - Unified Architecture Plan v4
...
> **Note:** Actual project naming uses `Orchestrator.*` / `NodeClient.*` prefixes, not `SplitBrain.*`. This reflects the separation between the orchestration host, its infrastructure libraries, and the per-provider node clients.

SplitBrain.AI/ ├── src/ │ ├── Orchestrator.Core/ # Domain models, interfaces, enums, DTOs, constants │ ├── Orchestrator.Infrastructure/ # NodeRegistry, RoutingService, HealthCheck, factory, LiteDB event log │ ├── Orchestrator.Agents/ # Semantic Kernel agent orchestration, state machine │ ├── Orchestrator.Mcp/ # MCP server, tool registration, idempotency │ ├── Orchestrator.NodeWorker/ ← ASP.NET Core host (primary node) │ ├── NodeClient.Ollama/ # OllamaInferenceNode, OllamaSharp integration │ ├── NodeClient.Copilot/ # CopilotInferenceNode, GitHub Copilot SDK │ ├── NodeClient.Worker/ # Lightweight HTTP relay client for remote nodes │ ├── SplitBrain.Dashboard/ # Blazor Server components, SignalR hub, 8 pages │ └── Orchestrator.Tests/ ← NUnit test project ├── Plans/ │ └── MasterPlanV4.md ← This document ├── deploy/ │ ├── setup-orchestrator.ps1 │ ├── setup-worker.ps1 │ └── publish/ ├── docs/ │ └── architecture-v2.md ├── scripts/ ├── config/ │ ├── appsettings.json # Models, fallback chains, routing, agents, MCP │ ├── appsettings.Development.json │ └── nodes.json ← Dynamic node topology (source of truth) ├── Orchestrator.slnx ├── .gitignore ├── LICENSE └── README.md

## 2.2 Project Responsibility Matrix

> Project names reflect the actual solution structure. Responsibilities are mapped to the existing `Orchestrator.*` / `NodeClient.*` / `SplitBrain.Dashboard` layout.

| **Project**                      | **Responsibility**                                                                                                                                                                                  | **Dependencies**                                 |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| **Orchestrator.Core**            | Domain models (NodeConfiguration, InferenceRequest, InferenceResult), interfaces (IInferenceNode, INodeRegistry), enums (NodeProviderType, NodeRole, TaskType, HealthState), DTOs, shared constants | None (leaf dependency)                           |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **Orchestrator.Infrastructure**  | NodeRegistry, RoutingService, NodeHealthCheckService, InferenceNodeFactory, FallbackChainResolver, LiteDbAgentEventLog, InMemoryMetricsCollector, FileLoggingService                                | Core, NodeClient.*, Polly, LiteDB                |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **NodeClient.Ollama**            | OllamaInferenceNode, OllamaSharp integration, VRAM health reporting                                                                                                                                 | Core, OllamaSharp                                |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **NodeClient.Copilot**           | CopilotInferenceNode, GitHub Copilot SDK integration, Key Vault auth chain                                                                                                                          | Core, GitHub.Copilot.SDK, Azure.Identity         |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **NodeClient.Worker**            | HTTP relay client for remote Ollama nodes — forwards inference and health requests to the remote Orchestrator.NodeWorker                                                                             | Core                                             |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **Orchestrator.Agents**          | Semantic Kernel integration, ChatCompletionAgent wrappers, bounded state machine (INIT→DONE\|FAIL), IAgentEventLog, step replay                                                                     | Core, Microsoft.SemanticKernel                   |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **Orchestrator.Mcp**             | MCP server via official SDK, tool registration (review_code, refactor_code, etc.), IIdempotencyCache, streamable HTTP + stdio transport                                                             | Core, Routing, Agents, ModelContextProtocol      |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **SplitBrain.Dashboard**         | Blazor Server Razor components, DashboardHub (SignalR), strongly-typed IDashboardClient, 8 pages: Overview, Nodes, Models, Tasks, Logs, Metrics, Settings, (NotFound). ApexCharts for charts.       | Core, Infrastructure, Agents, NodeClient.*       |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **Orchestrator.NodeWorker**      | ASP.NET Core host. DI composition root. Hosts MCP server, routing engine, Blazor dashboard, agent engine. Runs on the primary node.                                                                 | All libraries                                    |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
| **Orchestrator.Tests**           | NUnit test project covering core domain, routing logic, fallback chains, and integration scenarios                                                                                                  | All src projects                                 |
| ---                              | ---                                                                                                                                                                                                 | ---                                              |
...
> **⚠️ Implementation Gap:** `Orchestrator.Infrastructure/InferenceNodeFactory.cs` currently exists as an empty file. The factory below must be implemented — it is the critical missing piece that wires nodes.json → IInferenceNode instances. Without it, the NodeRegistry cannot create nodes dynamically and the system falls back to hardcoded node construction.

namespace SplitBrain.Networking;
...
| ---                     | ---               | ---                                                                       |
| Max tokens per loop     | **12,000**        | Per-loop budget ceiling (prompt + output combined). The model's 8K context window is the tighter runtime constraint — keep prompt + history + output within that. 12K is the hard kill switch if context accounting drifts or a larger-context model is used. |
| ---                     | ---               | ---                                                                       |
...
> **Implementation status:** All 8 page files exist. CSS design system is live in `wwwroot/app.css` with the dark palette described in §8.5.
...
| ---           | ---         | ---                             | ---                                                                                                                                                                     |
| **Logs**      | /logs       | Real-time structured log viewer | Filterable log stream (level, node, task, model), full-text search, log level badges, auto-scroll with pause. See §8.7 for full spec.                                   |
| ---           | ---         | ---                             | ---                                                                                                                                                                     |
| **Metrics**   | /metrics    | Charts and telemetry            | ApexCharts time-series (tokens/s, latency, VRAM), node comparison, task throughput. See §8.6 for full spec.                                                             |
| ---           | ---         | ---                             | ---                                                                                                                                                                     |
...
## 8.5 Dashboard Design System

> **Status: Implemented.** The CSS custom properties, dark palette, and component classes are live in `SplitBrain.Dashboard/wwwroot/app.css`.

The dashboard uses a custom dark theme defined via CSS custom properties on `:root`. The existing `sb-` CSS prefix is used throughout. Key design tokens:

| **Token**        | **Value**   | **Usage**                     |
| ---------------- | ----------- | ----------------------------- |
| `--sb-bg`        | `#0f1117`   | Page background               |
| `--sb-surface`   | `#1a1d27`   | Cards, nav, panels            |
| `--sb-surface2`  | `#232636`   | Table headers, node headers   |
| `--sb-border`    | `#2e3148`   | Borders, dividers             |
| `--sb-text`      | `#e2e8f0`   | Primary text                  |
| `--sb-muted`     | `#6b7280`   | Secondary text, placeholders  |
| `--sb-accent`    | `#6366f1`   | Active nav, badges, focus     |
| `--sb-green`     | `#22c55e`   | Healthy state, success        |
| `--sb-yellow`    | `#eab308`   | Degraded state, warning       |
| `--sb-red`       | `#ef4444`   | Unavailable state, error      |

Typography: `Segoe UI` for body text; `Cascadia Code` / `Consolas` for monospace (log viewer, code). Future light-mode support via a `data-theme="light"` attribute on `<body>` swapping the custom properties.

## 8.6 Charting with Blazor-ApexCharts

**Package:** `Blazor-ApexCharts` (MIT, .NET 8+, NuGet: `Blazor-ApexCharts`)

Selected for: free/MIT license, native Blazor components, `UpdateSeriesAsync()` for SignalR-driven live updates, built-in dark theme that matches the design system, and active maintenance on .NET 10.

### Setup

1. Add NuGet reference to `SplitBrain.Dashboard.csproj`: `<PackageReference Include="Blazor-ApexCharts" Version="1.*" />`
2. In `App.razor` (or `_Host.cshtml`): add `<script src="_content/Blazor-ApexCharts/js/blazor-apexcharts.js"></script>`
3. In `_Imports.razor`: `@using ApexCharts`
4. Register services in `Program.cs`: `builder.Services.AddApexCharts()`

### Chart Mapping per Page

| **Page**    | **Chart**                  | **Type**           | **Data Source**                        |
| ----------- | -------------------------- | ------------------ | -------------------------------------- |
| Overview    | Tokens/s sparklines        | Area (mini)        | `DashboardState.TokenUsage` (rolling)  |
| Metrics     | Tokens/s over time         | Area (multi-series)| `ReceiveTokenUsageUpdate` SignalR       |
| Metrics     | Inference latency          | Line (per node)    | `ReceiveNodeHealthUpdate` SignalR       |
| Metrics     | VRAM utilization           | Bar (stacked)      | `NodeHealthSnapshot.VramLoaded/Total`  |
| Metrics     | Task throughput            | Column (per type)  | `ReceiveTaskUpdate` SignalR             |
| Nodes       | Latency history (24h)      | Line               | Rolling `NodeHealthSnapshot` buffer    |

### Dark Theme Configuration

```csharp
// Applied once at chart construction
...
```csharp
// In a Blazor component, called from SignalR handler
private ApexChart<DataPoint> _chart = default!;

private async Task OnTokenUsageReceived(TokenUsageRecord record)
{
    _series[0].Data.Add(new DataPoint(record.Timestamp, record.TokensPerSecond));
    if (_series[0].Data.Count > 120) _series[0].Data.RemoveAt(0); // rolling 2-min window
...
The Logs page (`/logs`) renders live structured log entries streamed via SignalR. The current implementation renders a flat `@foreach` loop — this works but will degrade with high log volumes.

### Target UX

- **Color-coded level badges:** `Verbose`=muted gray, `Debug`=gray, `Information`=accent blue, `Warning`=yellow, `Error`=red. Applied via `.sb-level-{level}` CSS classes already in `app.css`.
- **Inline node/task tags** as pills beside each entry.
- **Collapsible exception stacks** via `<details>/<summary>`.
- **Auto-scroll with pause:** Scroll-to-bottom when not paused; freeze position when paused.
- **Virtualization (future):** Replace `@foreach` with `<Virtualize>` when entry count exceeds 500 to avoid DOM bloat. Buffer the last 2,000 entries in `DashboardState`.

### Auto-scroll Implementation

```razor
<div class="sb-log-list" @ref="_logContainer">
...
## 8.8 Sidebar Navigation Layout

The sidebar is implemented in `MainLayout.razor` using the `.sb-nav` fixed-left layout from `app.css`. Current state is functional. Target state adds icons and active-link highlighting.

### Nav Item Spec

Each `<NavLink>` in the sidebar:
- **Icon** (Unicode or SVG inline) + **Label** side by side
- `Match="NavLinkMatch.All"` on `/` (Overview only); `NavLinkMatch.Prefix` on all other routes
- `.active` class applies automatically via Blazor `NavLink`; `app.css` shows accent-left border
- Footer area shows app version + connection state badge (`●Connected` / `○Reconnecting`)
...
## 8.9 Node Health Cards

Node health cards are rendered on the Overview page via `.sb-node-card` / `.sb-node-grid` CSS. Current implementation is functional. Target enhancements:

### Card Anatomy (target)

```
┌─────────────────────────────────┐
│ ● RTX 5060 Laptop    [Healthy]  │  ← .sb-node-header with LED + name + badge
├─────────────────────────────────┤
│ Latency:  12 ms   Active:  1    │
│ VRAM:  ████░░░░  4.5 / 8 GB     │  ← progress bar (.sb-vram-bar)
│ Models:  [qwen2.5-coder:7b-q4]  │  ← pill badges (.sb-badge-running)
│ Sparkline: ▁▂▄▃▅▄▃▂▁           │  ← ApexCharts mini area (future)
└─────────────────────────────────┘
```

- **LED pulse animation** on Healthy: subtle CSS `@keyframes` glow pulse on `.sb-led`
- **VRAM bar turns red** when >90% (`sb-critical` class already in `app.css`)
- **Sparkline** (Phase 3+): small 60-point `ApexChart` area showing latency trend. Rendered only if data is available; gracefully absent on first load.
- **Error message** displayed in red below models when `Health.ErrorMessage` is set
...
| **Deliverable**                     | **Details**                                                                                                                    |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Blazor Server app                   | 8 pages: Overview, Nodes, Models, Tasks, Logs, Metrics, Settings, NotFound. SignalR DashboardHub. ✅ Page shells exist.         |
| ---                                 | ---                                                                                                                            |
| Design system                       | CSS custom property palette in `app.css` (sb- prefix). LED indicators, VRAM bars, node cards, alert strips. ✅ Implemented.     |
| ---                                 | ---                                                                                                                            |
| **InferenceNodeFactory** (CRITICAL) | Implement `Orchestrator.Infrastructure/InferenceNodeFactory.cs` — currently empty. Wires NodeConfiguration → IInferenceNode.  |
| ---                                 | ---                                                                                                                            |
| Node topology editor                | Settings page with Add/Edit/Remove node forms → saves to nodes.json → IOptionsMonitor hot-reload.                             |
| ---                                 | ---                                                                                                                            |
| Fallback chain editor               | Visual chain builder with drag-and-drop (or up/down buttons) reordering per TaskType.                                         |
| ---                                 | ---                                                                                                                            |
| Blazor-ApexCharts setup             | Add `Blazor-ApexCharts` NuGet to `SplitBrain.Dashboard`. Register services. Dark theme config. See §8.6.                      |
| ---                                 | ---                                                                                                                            |
| Metrics page charts                 | Tokens/s area chart, latency line chart, VRAM stacked bar, task throughput column — all live via `UpdateSeriesAsync`. See §8.6.|
| ---                                 | ---                                                                                                                            |
| Log viewer improvements             | Auto-scroll JS interop, pause button, level color badges, exception details collapse. Virtualize at 500+ entries. See §8.7.    |
| ---                                 | ---                                                                                                                            |
| Node health card sparklines         | Per-node mini ApexCharts latency sparkline on Overview/Nodes. LED pulse animation. See §8.9.                                  |
| ---                                 | ---                                                                                                                            |
...
**Success Criteria:** InferenceNodeFactory implemented and nodes.json drives live node creation. Can manage entire node topology from browser. Live health updates, log streaming, ApexCharts metrics visible in dashboard. Node sparklines show latency trend.
...
| ---                                          | ---                                                     | ---                                                                                   |
| **Blazor-ApexCharts**                        | **Blazor charting components**                          | **MIT. ApexCharts wrapper for Blazor. Use `UpdateSeriesAsync()` for live SignalR data. Dark theme built-in. See §8.6.** |
| ---                                          | ---                                                     | ---                                                                                   |
